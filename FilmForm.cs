...
namespace Kinoform
{
    public partial class FilmForm : Form
    {
        ...
        public FilmForm(int read, int kohad, string film)
        {
        ...
        {
            string film=ValiFilm.filmnimi;
            connect_to_DB.Open();
            string text = "Filmi näidatakse väikeses saalis,\nTe valite film " + film +",\nMe ootame teid Apollonos! ";
            foreach (var item in piletid)
            {
                text += "\nRida: " + item.Rida + " Koht: " + item.Koht + "\n";
                command = new SqlCommand("INSERT INTO vsaal(Rida,Koht,Film) VALUES (@rida,@koht,@film)", connect_to_DB);
        ...
        public string[] Ostetud_piletid()
        {
            ...
            {
                connect_to_DB.Open();
                if (ValiFilm.filmnimi == "Spiderman")
                {
                    adapter = new SqlDataAdapter("SELECT * FROM [dbo].[vsaal] WHERE Film = 'Spiderman' ", connect_to_DB);
                ...
                else if(ValiFilm.filmnimi == "Megamind")
                {
                    adapter = new SqlDataAdapter("SELECT * FROM [dbo].[vsaal] WHERE Film = 'Megamind' ", connect_to_DB);
                    DataTable tabel = new DataTable();
                    adapter.Fill(tabel);
                    read_kohad = new string[tabel.Rows.Count];
                    var index = 0;
