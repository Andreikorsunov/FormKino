...
namespace Kinoform
{
    public partial class FilmForm3 : Form
    {
        ...
        public FilmForm3(int read, int kohad, string film)
        {
        ...
        {
            string film=ValiFilm3.filmnimi;
            connect_to_DB.Open();
            string text = "Filmi näidatakse suures saalis,\nTe valite film " + film +",\nMe ootame teid Apollonos! ";
            foreach (var item in piletid)
            {
                text += "\nRida: " + item.Rida + " Koht: " + item.Koht + "\n";
                command = new SqlCommand("INSERT INTO ssaal(Rida,Koht,Film) VALUES (@rida,@koht,@film)", connect_to_DB);
        ...
        public string[] Ostetud_piletid()
        {
            ...
            {
                connect_to_DB.Open();
                if (ValiFilm3.filmnimi == "Superman")
                {
                    adapter = new SqlDataAdapter("SELECT * FROM [dbo].[ssaal] WHERE Film = 'Superman' ", connect_to_DB);
                ...
                else if(ValiFilm3.filmnimi == "Megamind")
                {
                    adapter = new SqlDataAdapter("SELECT * FROM [dbo].[ssaal] WHERE Film = 'Megamind' ", connect_to_DB);
                    DataTable tabel = new DataTable();
                    adapter.Fill(tabel);
                    read_kohad = new string[tabel.Rows.Count];
                    var index = 0;
                ...
                else if (ValiFilm3.filmnimi == "Batman")
                {
                    adapter = new SqlDataAdapter("SELECT * FROM [dbo].[ssaal] WHERE Film = 'Batman' ", connect_to_DB);
                    DataTable tabel = new DataTable();
                    adapter.Fill(tabel);
                    read_kohad = new string[tabel.Rows.Count];
                    var index = 0;
